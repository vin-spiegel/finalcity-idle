...
import { users, userResources, userExploration, userJobs } from "../db/schema.js";
...
init.get("/", requireAuth, async (c) => {
  ...
  const [zoneRows, userRow, resourceRows, expRow, jobRows] = await Promise.all([
    getZones(),
    ...
    db.select().from(userJobs).where(eq(userJobs.userId, userId)),
  ]);
  ...
  // skill level = jobPoints / 100  (0.00–100.00)
  const skills = Object.fromEntries(jobRows.map(j => [j.jobType, j.jobPoints / 100]));

  console.log(`[init] ${Date.now() - t0}ms userId=${userId}`);

  return c.json({
    success: true,
    data: { user: userRow, status, resources, skills, zones: zoneRows },
  });
});
